...
function enterprise_project_distribution_create(array $env, array $project, int $userId): array
{
    ...
    if (strtolower((string)($project['database_driver'] ?? 'mysql')) !== 'mysql') {
        throw new RuntimeException('Der HTML5-Anwenderpaket-Export ist derzeit für MySQL/MariaDB-Projekte freigegeben.');
    }
    ...
    try{
        $sqlFile=$tmp.'/database.sql'; $snapshot=false;
        ...
        $projectMeta=[
            ...
            'product_type'=>'dataform-html5','database_driver'=>'mysql','database_name'=>$database,'description'=>(string)($project['description']??''),
            'exported_at'=>gmdate('c'),'exported_by_user_id'=>$userId,
        ];
        $manifest=[
            'format'=>'easyit-dataform-html5-application','format_version'=>2,'runtime_version'=>'1.0','created_at'=>gmdate('c'),
            'project'=>$projectMeta,'dataforms'=>$forms,'database'=>$stats+['sha256'=>hash_file('sha256',$sqlFile)],
            'install'=>['entrypoint'=>'setup.php','requires'=>['PHP 8.1+','pdo_mysql','fileinfo'],'writes'=>'config.php'],
        ];
        ...
        try{
            ...
            enterprise_project_distribution_add_text($zip,$folder.'/config.php.example',"<?php\ndeclare(strict_types=1);\nreturn ['db_host'=>'localhost','db_port'=>3306,'db_name'=>'".addslashes($database)."','db_user'=>'','db_password'=>'','project_id'=>".$projectId.",'project_name'=>'".addslashes((string)$projectMeta['name'])."'];\n");$fileCount++;
