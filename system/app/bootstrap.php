...
function enterprise_upgrade(PDO $pdo): void {
    ...
    foreach ([['dataform','DataForm','available','5-dev'],['dialog','Dialog','planned',null],['nachhilfe','Nachhilfe','planned',null],['csv-engine','CSV-Engine','planned',null]] as $row) $stmt->execute($row);
}
...
function enterprise_can(array $user,string $capability): bool { return enterprise_is_admin($user) || in_array($capability,$user['permissions']??[],true); }
