...
final class ModuleMigrationAuditService
{
    ...
    {
        $ds=$this->state->getForProject('datasource.configure',$projectId,$projectId);$driver=strtolower((string)($ds['profile']['driver']??$ds['driver']??''));
        if(!in_array($driver,['mysql','oracle'],true))return ['status'=>'NOT_EXTERNAL','driver'=>$driver,'tables'=>[]];
        $pdoDriver=$driver==='oracle'?'oci':'mysql';if(!class_exists(\PDO::class)||!in_array($pdoDriver,\PDO::getAvailableDrivers(),true))return ['status'=>'SKIP','driver'=>$driver,'message'=>'PDO-Treiber ist für externen Schema-Snapshot nicht verfügbar.','tables'=>[]];
        $c=(array)($ds['connection']??[]);$password='';$ref=trim((string)($c['passwordRef']??''));if($ref!==''){$v=getenv($ref);if($v!==false)$password=(string)$v;}
        try{$opt=[\PDO::ATTR_ERRMODE=>\PDO::ERRMODE_EXCEPTION,\PDO::ATTR_DEFAULT_FETCH_MODE=>\PDO::FETCH_ASSOC];if($driver==='mysql'){$dsn=sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s',(string)($c['host']??'127.0.0.1'),(int)($c['port']??3306),(string)($c['database']??''),(string)($c['charset']??'utf8mb4'));$pdo=new \PDO($dsn,(string)($c['username']??''),$password,$opt);$rows=$pdo->query("SELECT table_name,column_name,column_type,is_nullable,column_key,ordinal_position FROM information_schema.columns WHERE table_schema=DATABASE() ORDER BY table_name,ordinal_position")->fetchAll();}else{$dsn=sprintf('oci:dbname=//%s:%d/%s;charset=%s',(string)($c['host']??'127.0.0.1'),(int)($c['port']??1521),(string)($c['service']??''),(string)($c['charset']??'AL32UTF8'));$pdo=new \PDO($dsn,(string)($c['username']??''),$password,$opt);$rows=$pdo->query("SELECT table_name,column_name,data_type,nullable,column_id FROM user_tab_columns ORDER BY table_name,column_id")->fetchAll();}$tables=[];foreach($rows as $r){$t=(string)($r['table_name']??$r['TABLE_NAME']??'');$col=(string)($r['column_name']??$r['COLUMN_NAME']??'');if($t===''||$col==='')continue;$tables[$t][]=$this->sanitizer->sanitize($r);}ksort($tables);return ['status'=>'PASS','driver'=>$driver,'tables'=>$tables];}catch(\Throwable $e){return ['status'=>'SKIP','driver'=>$driver,'message'=>'Externer Schema-Snapshot konnte nicht gelesen werden: '.$e->getMessage(),'tables'=>[]];}
    }
