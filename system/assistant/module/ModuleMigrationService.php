...
final class ModuleMigrationService
{
    ...
    private function pdo(string $driver,array $c,string $password):\PDO
    {
        ...
        if($driver==='pgsql'){$dsn=sprintf('pgsql:host=%s;port=%d;dbname=%s',(string)($c['host']??'127.0.0.1'),(int)($c['port']??5432),(string)($c['database']??''));$pdo=new \PDO($dsn,(string)($c['username']??''),$password,$opt);$pdo->exec("SET client_encoding TO 'UTF8'");return $pdo;}
