...
final class ProjectAssistant implements AssistantInterface
{
    ...
    public function getSteps(AssistantContext $context): array
    {
        return [
            ...
            new AssistantStep('storage', '2. Datenhaltung', 'Festlegen, ob die Projektdaten in MySQL, SQLite, CSV oder Oracle geführt werden.', [
                'fields' => [
                    ['name' => 'driver', 'label' => 'Datenhaltung', 'type' => 'select', 'options' => [
                        'mysql' => 'MySQL / MariaDB', 'sqlite' => 'SQLite', 'csv' => 'CSV', 'oracle' => 'Oracle',
                    ]],
                ],
            ]),
            new AssistantStep('datasource', '3. Datenquellenprofil', 'Startprofil für den Datenquellen-Assistenten erzeugen. Kennwörter werden hier nicht gespeichert.', [
                'fields' => [
                    ...
                    ['name' => 'database_name', 'label' => 'Datenbankname (MySQL)', 'type' => 'text'],
                ],
            ]),
    ...
    private function applyStepInput(ProjectDraft $draft, AssistantContext $context, string $stepId): ProjectDraft
    {
        ...
        if ($stepId === 'datasource') {
            ...
            if ($database === '' && ($d['storage']['driver'] ?? '') === 'mysql') { $database = (string) ($d['identity']['slug'] ?? ''); }
    ...
    private function derivePaths(ProjectDraft $draft): ProjectDraft
    {
        ...
        if ($driver === 'mysql' && $database === '') { $database = $slug; }
        return $draft->merge([
            ...
            'dataSource' => [
                ...
                'databaseName' => $database,
                'localPath' => $localPath,
                'requiresConnectionConfiguration' => in_array($driver, ['mysql', 'oracle'], true),
            ],
        ]);
    }
