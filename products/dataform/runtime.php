...
try {
    ...
    $projectPdo = new PDO(
        'mysql:host=' . ($env['PROJECT_DB_HOST'] ?? '127.0.0.1')
        . ';port=' . (int)($env['PROJECT_DB_PORT'] ?? 3306)
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_dataform_settings') {
        ...
        $eventKeys=['open','close','record_change','before_save','after_save','before_delete','after_delete'];
        $events=[];
        foreach($eventKeys as $eventKey){$code=trim((string)($_POST['event_'.$eventKey]??''));if(strlen($code)>8000)throw new RuntimeException('Ein Event-Handler ist zu lang.');$events[$eventKey]=$code;}
    ...
    if (
        ...
        $info='Die physische Spalte id bleibt technischer Primärschlüssel und wird nicht als normales Formularfeld angelegt.';
    }
    ...
    <div class="df-workspace-grid">
        ...
        <main class="df-editor" aria-label="Arbeitsbereich">
            ...
            <div class="df-editor-content">
                ...
                <?php elseif ($sectionKey === 'sources'): $sourceCfg=$selectedSource?DataSourceManager::config($selectedSource):[]; $sourceDriver=(string)($selectedSource['driver']??'mysql'); ?>
                    ...
                    <section class="card df-system-source">
                        ...
                        <dl class="status-list">
                            <div><dt>Treiber</dt><dd>MySQL / MariaDB</dd></div>
                            <div><dt>Datenbank</dt><dd><code><?= e((string)$project['database_name']) ?></code></dd></div>
                            <div><dt>Host</dt><dd><code><?= e((string)($env['PROJECT_DB_HOST']??'127.0.0.1')) ?>:<?= (int)($env['PROJECT_DB_PORT']??3306) ?></code></dd></div>
                        </dl>
                    </section>
                    ...
                    <div class="df-source-layout">
                        <section>
                            ...
                            <div class="df-source-list">
                                ...
                                <article class="df-source-card <?= !$source['is_enabled']?'disabled':'' ?>">
                                    ...
                                    <?php if($source['driver']==='mysql'): ?><?= e((string)($cfg['host']??'')) ?>:<?= (int)($cfg['port']??3306) ?> / <code><?= e((string)($cfg['database']??'')) ?></code>
                            ...
                            <?php else: ?><div class="empty-state"><h3>Noch keine zusätzliche Datenquelle</h3><p>Die Projekt-Datenbank ist bereits aktiv. Ergänzen Sie hier MySQL/MariaDB, SQLite, CSV oder Oracle.</p></div><?php endif; ?>
                        </section>
                        ...
                        <section class="card" id="source-form">
                            ...
                            <form method="post" data-source-form>
                                ...
                                <div data-source-driver-fields="mysql oracle">
                                    <div class="df-form-grid">
                                        <label><strong>Host</strong><input name="host" value="<?= e((string)($sourceCfg['host']??'127.0.0.1')) ?>"></label>
                                        <label><strong>Port</strong><input type="number" min="1" max="65535" name="port" value="<?= (int)($sourceCfg['port']??($sourceDriver==='oracle'?1521:3306)) ?>"></label>
                                        <label data-driver-only="mysql"><strong>Datenbank</strong><input name="database" value="<?= e((string)($sourceCfg['database']??'')) ?>"></label>
                                        <label data-driver-only="mysql"><strong>Charset</strong><input name="charset" value="<?= e((string)($sourceCfg['charset']??'utf8mb4')) ?>"></label>
                                        <label data-driver-only="oracle"><strong>Service-Name</strong><input name="service_name" value="<?= e((string)($sourceCfg['service_name']??'FREEPDB1')) ?>"></label>
                    ...
                    (function(){
                        ...
                        function sync(){
                            ...
                            form.querySelectorAll('[data-driver-only]').forEach(function(field){field.hidden=field.getAttribute('data-driver-only')!==value;});
                            var port=form.querySelector('input[name="port"]');
                            if(port && !port.dataset.touched) port.value=value==='oracle'?'1521':'3306';
                        }
                        var port=form.querySelector('input[name="port"]');if(port)port.addEventListener('input',function(){port.dataset.touched='1';});
                ...
                <?php elseif ($sectionKey === 'tables'): ?>
                    ...
                    <section class="card df-table-source-bar">
                        <form method="get" class="df-table-source-form">
                            ...
                            <span class="badge"><?= $tableSourceKey==='system'?'Systemquelle · Schreibzugriff':'Externe Quelle · Nur Lesen' ?></span>
                        </form>
                    </section>
                            ...
                            <section class="card df-table-detail">
                                <div class="df-toolbar">
                                    ...
                                    <div class="actions df-table-actions">
                                        ...
                                        <a class="button" href="?project=<?= (int)$project['id'] ?>&amp;section=designer&amp;dataform=<?= (int)$tableDataformBinding['dataform_id'] ?>">
                                        ...
                                        <form method="post" onsubmit="return confirm('Aus der Tabelle <?= e($selectedTableName) ?> jetzt ein DataForm erzeugen?');">
                                            ...
                                            <button class="button" type="submit">DataForm aus Tabelle erstellen</button>
                                        </form>
                                        <?php endif; ?>
                                        <?php if(!empty($tableInspection['managed'])): ?>
                                        <form method="post" onsubmit="return confirm('Die Tabelle <?= e($selectedTableName) ?> und alle enthaltenen Daten wirklich löschen?');">
                                            <input type="hidden" name="csrf" value="<?= e(enterprise_csrf()) ?>">
                                            <input type="hidden" name="project" value="<?= (int)$project['id'] ?>">
                                            <input type="hidden" name="section" value="tables">
                                            <input type="hidden" name="action" value="drop_managed_table">
                                            <input type="hidden" name="table_source" value="system">
                                            <input type="hidden" name="table" value="<?= e($selectedTableName) ?>">
                                            <button class="link-danger" type="submit">Tabelle löschen</button>
                                        </form>
                                        <?php endif; ?>
                                    </div>
                                    <?php endif; ?>
                                ...
                                <div class="notice info">
                                    ...
                                    Die Tabellenherkunft ist persistent gespeichert.
                                </div>
                                <?php else: ?>
                                <div class="notice info">
                                    Für diese Projekttabelle existiert noch kein DataForm. Es kann direkt aus der aktuellen Spaltendefinition erzeugt werden.
                                </div>
                                <?php endif; ?>
                                <?php endif; ?>
                                ...
                                <div class="df-toolbar df-column-toolbar">
                                    ...
                                    <?php if($tableSourceKey==='system' && !empty($tableInspection['field_crud'])): ?>
                                    <a class="button secondary" href="?project=<?= (int)$project['id'] ?>&amp;section=tables&amp;table_source=system&amp;table=<?= rawurlencode($selectedTableName) ?>#field-crud">+ Feld hinzufügen</a>
                                    <?php endif; ?>
                                </div>
                                    ...
                                    <table class="df-field-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <?php if($tableSourceKey==='system' && !empty($tableInspection['field_crud'])): ?><th>Aktionen</th><?php endif; ?>
                                            </tr>
                                        </thead>
                                        <tbody>
                                ...
                                <article class="df-form-item">
                                    <div>
                                        ...
                                        <div class="df-form-meta">
                                            ...
                                            <?php if(isset($dataformBindingsById[(int)$form['id']])): ?>
                                            · Tabelle: <code><?= e((string)$dataformBindingsById[(int)$form['id']]['table_name']) ?></code>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                    ...
                    <section class="card" id="dataform-settings">
                        ...
                        <form method="post" data-dataform-settings-form action="?project=<?= (int)$project['id'] ?>&amp;section=dataform&amp;dataform=<?= (int)$selectedDataform['id'] ?>#dataform-settings">
                            ...
                            <div class="df-settings-guide" aria-label="Einstellungsgruppen">
                                ...
                                <a href="#settings-events"><strong>Runtime-Events</strong><span>JavaScript-Hooks</span></a>
                            </div>
                            <div class="df-settings-sections">
                              ...
                              <fieldset class="df-setting-group" id="settings-events"><legend>5 · Runtime-Events (JavaScript)</legend><p class="muted"><strong>Abgrenzung:</strong> Diese Hooks laufen direkt in der Runtime. Deklarative Formularregeln gehören zu <a href="foundation.php?project=<?= (int)$project['id'] ?>&amp;dataform=<?= (int)$selectedDataform['id'] ?>&amp;mode=behavior">Verhalten</a>; fachliche Status und Übergänge zu <a href="workflow.php?project=<?= (int)$project['id'] ?>&amp;dataform=<?= (int)$selectedDataform['id'] ?>">Workflow</a>.</p><div class="df-form-grid">
                                <?php $eventHandlers=json_decode((string)($selectedDataform['event_handlers_json']??''),true); if(!is_array($eventHandlers))$eventHandlers=[]; foreach(['open'=>'Öffnen','close'=>'Schließen','record_change'=>'Datensatzwechsel','before_save'=>'Vor Speichern','after_save'=>'Nach Speichern','before_delete'=>'Vor Löschen','after_delete'=>'Nach Löschen'] as $ek=>$el): ?>
<label class="full"><strong><?=e($el)?></strong><textarea name="event_<?=e($ek)?>" rows="3" spellcheck="false"<?= $ek==='after_save'?' placeholder="afterSave(dataformContext);"':'' ?>><?=e((string)($eventHandlers[$ek]??''))?></textarea>
<?php if(in_array($ek,['before_save','before_delete'],true)): ?><small><code>return false;</code> oder <code>dataformContext.event.cancel = true;</code> kann die Aktion abbrechen.</small><?php elseif($ek==='after_save'): ?><small>Beispiel: <code>afterSave(dataformContext);</code> · <code>dataformContext.record.values</code> enthält den tatsächlich gespeicherten Datensatz, <code>record.original_values</code> den vorherigen Stand und <code>record.changes</code> nur die Änderungen.</small><?php else: ?><small>Im Handler steht das benannte Übergabeobjekt <code>dataformContext</code> zur Verfügung.</small><?php endif; ?></label><?php endforeach; ?>
<details class="full df-action-context-help"><summary>DataFormActionContext 1.0 – Übergabeobjekt anzeigen</summary><p>Alle JavaScript-Events erhalten das Objekt <code>dataformContext</code>. Für „Nach Speichern“ ist es ein serverseitiger Snapshot nach erfolgreichem INSERT/UPDATE.</p><pre>{ schema, action, project, dataform, context, record: { id, values, original_values, changes, dirty_fields }, fields, validation, ui, event, result }</pre></details>
                              </div></fieldset>
                            </div>
                            <?php if(isset($dataformBindingsById[(int)$selectedDataform['id']])): ?>
                            <div class="notice info"><strong>Physische Tabellenbindung:</strong> <code><?= e((string)$dataformBindingsById[(int)$selectedDataform['id']]['table_name']) ?></code>. Die Tabellenbindung wird hier nur angezeigt und nicht versehentlich verändert.</div>
                            <?php endif; ?>
