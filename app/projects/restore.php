            ...
            try {
                ...
                if (($dbResult['database_restored'] ?? false) && !($dbResult['database_existed_before'] ?? false)) {
                    try {
                        ...
                        $server->exec('DROP DATABASE IF EXISTS ' . enterprise_project_restore_quote_identifier($form['database_name']));
                    } catch (Throwable) {
                    }
                }
...
<section class="hero">
    ...
    <p>Laden Sie eine mit dem Enterprise Manager erzeugte Projekt-ZIP-Sicherung hoch. Das Archiv wird zuerst geprüft; erst danach entscheiden Sie, wie Projektregistrierung und Projektdatenbank wiederhergestellt werden.</p>
</section>
...
<section class="card">
    ...
    <form method="post" enctype="multipart/form-data" class="form-grid">
        ...
        <label class="form-span">
            ...
            <small>Erwartet wird das ZIP aus „Projekt vor dem Löschen als Download sichern“ mit <code>manifest.json</code>, <code>project.json</code> und <code>database.sql</code>.</small>
        </label>
...
<section class="card">
    ...
    <dl class="status-list">
        ...
        <div><dt>Ursprüngliche Datenbank</dt><dd><code><?= e($sourceDb) ?></code></dd></div>
...
<section class="card">
    ...
    <form method="post" class="form-grid" id="project-restore-form">
        ...
        <label>
            ...
            <small>Standardmäßig wird der Datenbankname aus der Sicherung verwendet.</small>
        </label>
        <label>
            Projektstatus
            <select name="status">
        ...
        <label class="form-span">
            Verhalten für die Projektdatenbank
            <select name="database_strategy" id="database_strategy">
                ...
                <option value="reuse" <?= $form['database_strategy'] === 'reuse' ? 'selected' : '' ?>>Vorhandene Datenbank verwenden – SQL-Dump nicht einspielen</option>
                <option value="restore" <?= $form['database_strategy'] === 'restore' ? 'selected' : '' ?>>Datenbank aus Sicherung wiederherstellen – Ziel darf noch nicht existieren</option>
                <option value="replace" <?= $form['database_strategy'] === 'replace' ? 'selected' : '' ?>>Vorhandene Datenbank endgültig ersetzen – DROP + Restore</option>
            </select>
            <small><strong>Empfehlung:</strong> „Automatisch“. Wurde beim Löschen nur die Projektregistrierung entfernt, bleibt die vorhandene Datenbank unverändert. Wurde die Datenbank mitgelöscht, wird sie aus dem ZIP neu aufgebaut.</small>
        </label>

        <div class="form-span notice error" id="replace-warning" hidden>
            <strong>Achtung – destruktiver Restore:</strong> Die vorhandene Ziel-Datenbank wird vor dem Restore vollständig gelöscht. Dieser Schritt kann nicht rückgängig gemacht werden.
            <label style="margin-top:.75rem">Datenbankname zur Bestätigung
        ...
        <div class="form-span notice">
            <strong>Sicherheitslogik:</strong> System- und Enterprise-Datenbanken sind als Restore-Ziel gesperrt. Slug und Datenbank dürfen keinem anderen registrierten Projekt gehören. Bei einem neuen Ziel wird eine unvollständige Datenbank nach Restore-Fehlern automatisch entfernt.
        </div>
...
render_page([
    ...
    'help' => [
        ...
        'goal' => 'Projektregistrierung und – falls erforderlich – die physische Projektdatenbank aus dem ZIP wiederherstellen.',
        'steps' => [
            'Projekt-ZIP auswählen und zuerst prüfen.',
            'Projektname, Slug und Ziel-Datenbank kontrollieren.',
            'Datenbankstrategie festlegen.',
            'Projekt wiederherstellen.',
        ],
        'tips' => [
            '„Automatisch“ verwendet eine noch vorhandene Originaldatenbank weiter und spielt nur dann SQL ein, wenn die Datenbank fehlt.',
            '„Vorhandene Datenbank ersetzen“ löscht die Zieldatenbank unwiderruflich und verlangt deshalb die exakte Bestätigung des Datenbanknamens.',
