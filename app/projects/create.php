...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(!in_array($projectDriver,['mysql','csv','sqlite','pgsql'],true)) throw new RuntimeException('Der konfigurierte Projektdatenspeicher wird in dieser Phase nicht unterstützt: '.$projectDriver);
        if(enterprise_project_store_exists($env,$form['database_name'],$projectDriver)) {
            throw new RuntimeException('Der Projektdatenspeicher `'.$form['database_name'].'` existiert bereits. Verwenden Sie „Vorhandenes Projekt registrieren“ oder wählen Sie einen neuen Namen.');
        }

        $created=false;
        try {
            $projectPdo=enterprise_project_store_create($env,$form['database_name'],$projectDriver);
            $created=true;
            ...
            if($projectDriver==='csv'){
                $sourceId=enterprise_project_store_ensure_default_csv_source($projectPdo,$env,$form['database_name']);
                $schemaResults[]='DEFAULT CSV DATA SOURCE #'.$sourceId;
            }elseif($projectDriver==='sqlite'){
                $sourceId=enterprise_project_store_ensure_default_sqlite_source($projectPdo,$env,$form['database_name']);
                $schemaResults[]='DEFAULT SQLITE DATA SOURCE #'.$sourceId;
            }
            elseif($projectDriver==='pgsql'){
                $sourceId=enterprise_project_store_ensure_default_pgsql_source($projectPdo,$env,$form['database_name']);
                $schemaResults[]='DEFAULT POSTGRESQL DATA SOURCE #'.$sourceId;
            }

            // Projekt erst nach erfolgreicher Speicher- und Schema-Provisionierung registrieren.
            $stmt=$pdo->prepare("INSERT INTO projects(name,slug,product_type,database_driver,database_name,status) VALUES (?,?,?,?,?,'active')");
            $stmt->execute([$form['name'],$form['slug'],$form['product_type'],$projectDriver,$form['database_name']]);
            ...
            enterprise_audit($pdo,(int)$user['id'],'project.provision','project',(string)$id,[
                ...
                'database_driver'=>$projectDriver,'database_name'=>$form['database_name'],'database_created'=>true,'schema'=>$schemaResults,
            ]);
            enterprise_event_dispatch('project.provisioned',[
                'project_id'=>$id,'name'=>$form['name'],'slug'=>$form['slug'],'product_type'=>$form['product_type'],
                'database_driver'=>$projectDriver,'database_name'=>$form['database_name'],
            ],['user_id'=>(int)$user['id']]);

            header('Location: view.php?id='.$id);
            exit;
        } catch(Throwable $e) {
            if($created){ try{ enterprise_project_store_delete($env,$form['database_name'],$projectDriver); }catch(Throwable){} }
            throw $e;
        }
    }
} catch(Throwable $e) {
...
<label>Datenbank-/Speichername<input name="database_name" required pattern="[A-Za-z][A-Za-z0-9_]{1,62}" value="<?=e($form['database_name'])?>" placeholder="easyit_testprojekt_63"><small>Der Projektdatenspeicher muss noch nicht existieren.</small></label>
